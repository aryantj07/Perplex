...
page = st.sidebar.radio("Select Page", ["Debiasing", "Train & Evaluate", "Scorecard"])
...
if st.session_state.page == "Scorecard":
    ...
    # Optional: Upload metrics CSV if missing in session (Advanced, only needed for full persistence)
    # Uncomment below if you want it
    # st.markdown("#### (Advanced) Upload metrics CSV to restore results")
    # uploaded_metrics_file = st.file_uploader(
    #     "Upload metrics CSV (optional)", type=["csv"], key="metrics_csv_upload")
    # if uploaded_metrics_file is not None:
    #     import pandas as pd
    #     metrics_df = pd.read_csv(uploaded_metrics_file, index_col=0)
    #     for idx, row in metrics_df.iterrows():
    #         if idx == "Baseline (Raw)":
    #             st.session_state["raw_metrics"] = row.to_dict()
    #         elif idx == "After Debiasing":
    #             st.session_state["debiased_metrics"] = row.to_dict()
    #         st.session_state["performance_compare"] = metrics_df

    # Must have trained models/metrics (raw baseline)
    if "raw_metrics" not in st.session_state:
        ...
        if not have_after:
            ...
            if "performance_compare" in st.session_state:
                ...
                st.session_state["performance_compare"] = pc
        ...
        if scorecard is None:
            ...
            if st.button("Generate Scorecard PDF"):
                try:
                    demo_metrics = {}
                    if "performance_compare" in st.session_state:
                        demo_metrics = {f"{idx}_{k}": float(v)
                                        for idx, row in st.session_state["performance_compare"].iterrows()
