...
- Community backend for crowdsourcing (unchanged)
...
# ---------------------------
# Light Professional UI (Blue Accent)
# ---------------------------
...
menu = st.sidebar.radio("Go to", [
    ...
    "Community"
          ...
          "Train & Evaluate","Scorecard","Simulator","Drift","Community"].index(st.session_state.get("page", "Dashboard")))
...
if st.session_state.page == "Debiasing":
    ...
    st.caption("Reweighing makes **S (sensitive)** and **Y (label)** more independent. Create a *debiased dataset* (via weighted resampling) or compute *sample weights* to use during model training.")
    if df is None:
        st.warning("Upload dataset first.")
    else:
        cols = list(df.columns)
        sensitive_candidates = detect_sensitive_attributes(df)
        sensitive = st.selectbox("Sensitive attribute", sensitive_candidates + cols if sensitive_candidates else cols, key="deb_sensitive")
        ...
        if run:
            if mitigation is None:
                ...
                try:
                    deb_df, weights, report = mitigation.debias_dataset(df, sensitive, target, strategy=strat_key, random_state=42)

                    st.success("Debiasing completed.")
                    st.write("**Report:**", report)

                    # NEW: persist debiased dataset for pipeline
                    ...
                    # also reset downstream models/metrics when new debiasing is produced
                    ...
                    # Offer download
                    st.download_button("Download debiased CSV", data=df_to_csv_bytes(deb_df), file_name="debiased_dataset.csv", mime="text/csv")

                    # Save weights for training page
...
if st.session_state.page == "Train & Evaluate":
    ...
    if df is None:
        ...
        # If we computed reweighing weights earlier, offer to use them
        use_weights = False
        if st.session_state.get("reweigh_weights") is not None:
            wi = st.session_state.get("weights_info", {})
            st.info(f"Reweighing weights available (strategy: {wi.get('strategy','n/a')}, sensitive: {wi.get('sensitive','?')}, target: {wi.get('target','?')}). You can train with these weights.")
            use_weights = st.checkbox("Use stored reweighing sample weights in training (where supported)", value=True)
        ...
        if st.button("Train & Evaluate"):
            try:
                # detect type
                detected = "Regression" if (problem_type == "Regression" or (problem_type == "Auto-detect" and pd.api.types.is_numeric_dtype(df[target]))) else "Classification"
                st.info(f"Detected problem type: {detected}")

                # 1. Train on RAW dataset (always baseline)
                raw_src = st.session_state.get("raw_dataset", df)
                raw_metrics, raw_model, raw_preds, raw_ytest = _train_eval_on_df(raw_src, target, detected, sensitive if exclude_sensitive else None)
                ...
                # 2. Train on DEBIASED dataset (if exists)
                if st.session_state.get("debiased_dataset") is not None:
                    deb_src = st.session_state["debiased_dataset"]
                    deb_metrics, deb_model, deb_preds, deb_ytest = _train_eval_on_df(deb_src, target, detected, sensitive if exclude_sensitive else None)
                    ...
                    st.info("No debiased dataset found. Run Debiasing first for a Before/After comparison.")
...
# ---------------------------
# Scorecard page  (ENHANCED: adds full automated report)
# ---------------------------
if st.session_state.page == "Scorecard":
    start_card()
    st.subheader("Generate Ethical Scorecard (True Before/After)")

    # Guardrails: must have trained models/metrics
    if "raw_metrics" not in st.session_state:
        st.warning("Please run **Train & Evaluate** first (on the Raw dataset).")
    else:
        have_after = "debiased_metrics" in st.session_state

        if not have_after:
            st.info("Debiased results not found. Run **Debiasing** and then **Train & Evaluate** again to compare Before vs After.")
            # Show at least the baseline
            st.write("### Baseline Metrics (Raw dataset)")
            st.json({k: round(v, 4) for k, v in st.session_state["raw_metrics"].items()})
        else:
            st.write("### Before vs After (Raw vs Debiased)")
            if "performance_compare" in st.session_state:
                st.dataframe(st.session_state["performance_compare"], use_container_width=True)
            else:
                pc = pd.DataFrame({
                    "Baseline (Raw)": st.session_state["raw_metrics"],
                    "After Debiasing": st.session_state["debiased_metrics"]
                })
                st.dataframe(pc, use_container_width=True)

            # Composite scores (simple)
            def _score(m: dict):
                if "Accuracy" in m and "F1" in m:
                    return int((m["Accuracy"]*0.5 + m["F1"]*0.5)*100)
                if "R2" in m:
                    # map R2 ~[0..1] to 0..100
                    return int(max(0.0, min(1.0, m["R2"])) * 100)
                return int(np.mean(list(m.values()))*100) if m else 0

            score_raw = _score(st.session_state["raw_metrics"])
            score_deb = _score(st.session_state["debiased_metrics"])

            c1, c2 = st.columns(2)
            with c1:
                st.metric("Score (Raw)", f"{score_raw}/100")
            with c2:
                st.metric("Score (Debiased)", f"{score_deb}/100")

            # Simple bar comparison
            try:
                import matplotlib.pyplot as plt
                labels = list(st.session_state["raw_metrics"].keys())
                raw_vals = [st.session_state["raw_metrics"][k] for k in labels]
                deb_vals = [st.session_state["debiased_metrics"].get(k, None) for k in labels]
        ...
        # Optional PDF scorecard using backend if available
        st.write("---")
        if scorecard is None:
            st.info("Scorecard backend missing.")
        else:
            if st.button("Generate Scorecard PDF"):
                try:
                    demo_metrics = {}
                    if "performance_compare" in st.session_state:
                        # Flatten a little for the backend
                        demo_metrics = {f"{idx}_{k}": float(v) for idx, row in st.session_state["performance_compare"].iterrows() for k, v in row.items()}
                    else:
                        demo_metrics = {"raw_"+k: float(v) for k, v in st.session_state.get("raw_metrics", {}).items()}
                        demo_metrics.update({"deb_"+k: float(v) for k, v in st.session_state.get("debiased_metrics", {}).items()})
...
if st.session_state.page == "Community":
    ...
    if community is None:
        ...
        if subs.empty:
            ...
            st.dataframe(subs.sort_values("timestamp", ascending=False).head(20), use_container_width=True)
